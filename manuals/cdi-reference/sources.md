# CDI / Spark Unit Sources

Checked: 2026-04-29

## Local Project Sources

- `../electrical-reference/manuals/01-ignition-system.pdf`
- `../electrical-reference/manuals/01-ignition-system.txt`
- `../carburetor-reference/manuals/03-shop-manual-maintenance.txt`
- `../electrical-reference/manuals/04-wiring-diagram-1982-750-sabre.pdf`
- `../electrical-reference/notes/common-issues.md`
- `../electrical-reference/notes/internet-sources.md`
- `../../HowTo/README.md`
- `../../problems.md`

## Web Sources

### Primary Product Sources

- Ignitech product overview
  https://www.ignitech.cz/en/vyrobky

- Ignitech SPARKER TCI
  https://www.ignitech.cz/en/vyrobky/tci/tci.htm

- Ignitech TCIP4
  https://www.ignitech.cz/en/vyrobky/tcip/tcip.htm

- Ignitech manual list for TCI applications
  https://www.ignitech.cz/en/vyrobky/tci/mototci.htm

- Ignitech DCCDIP4
  https://www.ignitech.cz/en/vyrobky/dccdip4/dccdip4.htm

- Carmo VF700 / VF750 replacement igniter
  https://www.carmo.nl/index.php?cPath=4318_4321_492_4288_20_1112_259&language=en&main_page=product_info&products_id=1921

- Carmo VF750S / VF750 / VF700F / VF700C V45 Sabre category
  https://www.carmo.nl/index.php?cPath=493_4686_1550_1764&language=en&main_page=index

- Carmo VF700 / VF750 replacement igniter pair
  https://www.carmo.nl/index.php?main_page=product_info&products_id=1940

- Rae-San PULSER TAI Ignition Module
  https://www.rae-san.com/rae-san-pulser-tai-ignition-module-2/

### V4MuscleBike Forum Sources

- Here we go again
  https://v4musclebike.com/threads/here-we-go-again.46903/
  Direct V45 Sabre thread with Ignitech TCIP4 installation, hot-failure diagnosis, pulse-generator warnings, and fuel-system clarification.

- V4spark.com
  https://v4musclebike.com/forums/showthread.php?p=531158
  Useful for the state of V4spark / Dualcam availability and alternate vendor names.

- V65 CDI box
  https://v4musclebike.com/forums/showthread.php?t=47554
  Useful Rae-San V65 discussion and reminder that the CDI boxes do not control the fuel pump.

- Ideal ignition setup on V65
  https://v4musclebike.com/threads/ideal-ignition-setup-on-v65.45321/
  Useful system-level discussion of coil power, COP, Rae-San, plug caps, and why the platform is not a classic CDI system.

- Ignitech replacement for Dyna 3000
  https://v4musclebike.com/threads/ignitech-replacement-for-dyna-3000.40044/
  Later-Magna thread, not a first-gen Sabre source, but useful for the TCIP4 programming workflow, adapter/cable notes, and stock-like map discipline.

- no spark at front cylinders
  https://v4musclebike.com/showthread.php?t=5602
  Older diagnostic thread for two-cylinder / one-bank no-spark logic.

## Interpretation Notes

- The local Honda / Haynes material is used for stock specs and timing checkpoints.
- Ignitech is used as the primary source for modern TCI vs CDI product distinctions.
- Carmo is used as the primary source for currently listed off-the-shelf replacement-module availability.
- Rae-San is treated as a vendor-contact path, not as proven VF750S plug-in compatibility.
- Forum material remains useful for symptom patterns and owner experience, but it is not treated as the primary source for hardware compatibility.
